...
for (i in 1:length(dat$observation)){
  ...
  dat$lower[i] <- dat$estimate[i] - qchisq(0.025, 2*dat$n_size[i])/(2*dat$n_size[i]*(1/dat$estimate[i]))
  
  dat$upper[i] <- dat$estimate[i] + qchisq(0.025, 2*dat$n_size[i])/(2*dat$n_size[i]*(1/dat$estimate[i]))
...
dat_exact_paramter <- data.frame(parameter = seq(0.1, 20, length.out = 100))

for (i in 1:length(dat_exact_paramter$parameter)) {

  dat_exact_paramter$cov_prob[i] <- exact_ci(1000, dat_exact_paramter$parameter[i], 50)
}
...
dat_delta_n <- data.frame(n_size = seq(10, 500, 1))
...
dat_delta_n %>%
  ...
  geom_smooth(aes(x = n_size, y = cov_prob))
...
dat_exact_n <- data.frame(n_size = seq(10,500, 1))
...
dat_exact_n %>%
  ggplot() +
  geom_path(aes(x = n_size, y = cov_prob))
...
ggplot() +
  geom_smooth(data = manyR50, aes(x = rate, y = coverages), col = bayesian) +
  geom_smooth(data = dat_delta_parameter, aes(x = parameter, y = cov_prob), col = delta)
...
```{r}
dat_n <- data.frame(n_size = seq(10, 500, 1))
